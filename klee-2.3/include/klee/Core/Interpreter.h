...
public:
  ...
  virtual void incErrorPaths() = 0;
  virtual void incLeakyPaths() = 0;
...
public:
  ...
  virtual void initializeSolver(std::string sytInitConstraints) = 0;
  ...
  // supply a filename for SyT-Klee to dump post-execution constaints into
  virtual void setSytPostConstraints(std::string filename) = 0;
  ...
  virtual void runFunctionInSyt(llvm::Function *f) = 0;

  virtual void readTPotConfig(std::string filename) = 0;

  virtual void applyInvariantRestrictions(const std::string &mainFnName) {};
  ...
  virtual void setSmokeTest(bool value) = 0;

  virtual void setMaxSmokeTestCount(unsigned int value) = 0;

  virtual void setAPIFunctions(std::vector<std::string> apiFunctions) = 0;
};
