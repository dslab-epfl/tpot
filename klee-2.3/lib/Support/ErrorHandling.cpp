...
static const char *tpotMsgPrefix = "TPOT";
static const char *tpotOptDebugPrefix = "TPOT_OPT";
...
static void klee_vfmessage(FILE *fp, const char *pfx, const char *msg,
  ...
  if (modifyConsoleColor) {
    ...
    // TPOT messages
    if (shouldSetColor(pfx, msg, tpotMsgPrefix))
      fdos.changeColor(llvm::raw_ostream::YELLOW,
                       /*bold=*/true,
                       /*bg=*/false);
                       
    if (shouldSetColor(pfx, msg, tpotOptDebugPrefix))
...
void klee::tpot_message(const char *msg, ...) {
  va_list ap;
  va_start(ap, msg);
  klee_vmessage(tpotMsgPrefix, WarningsOnlyToFile, msg, ap);
  va_end(ap);
}

void klee::tpot_opt_debug(const char *msg, ...) {
  va_list ap;
  va_start(ap, msg);
  klee_vmessage(tpotOptDebugPrefix, WarningsOnlyToFile, msg, ap);
  va_end(ap);
}
