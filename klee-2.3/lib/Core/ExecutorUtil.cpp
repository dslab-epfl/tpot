...
namespace klee {
  ...
  ref<klee::Expr> Executor::evalConstantTpotWrapper(const Constant *c,
                                                 const KInstruction *ki) {
    if (const GlobalValue *gv = dyn_cast<GlobalValue>(c)) {
      auto it = globalAddressesTpotWrapper.find(gv);
      if (it != globalAddressesTpotWrapper.end()) {
        return it->second;
      }
    }
    auto evalResult = evalConstant(c, ki);
    // look up the substitution table for global addresses.
    auto it = tpotGlobalAddressSubst.find(evalResult->getZExtValue());
    if (it != tpotGlobalAddressSubst.end()) {
      return it->second;
    }
    return evalResult;
  }
