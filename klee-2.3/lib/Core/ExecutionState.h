...
enum LambdaPurpose {MEMOIZE, INSTANTIATE, CHECK};

struct InvHavocLevel {
  KInstruction *ki;
  Heap *heap;
  AddressSpace *addressSpace;
  bool nonHavocedModified;
};
...
public:
  ...
  /// @brief Address space used by this state (e.g. Global and Heap) - only Global in SyT.
  AddressSpace addressSpace;

  /// @brief SyT maintains the heap separately.
  Heap heap;

  //std::vector<ObjectPair>  uniqueHeaps;

  /// @brief Constraints collected so far
  ConstraintSet constraints;

  ConstraintSet constraints_heapOnly;
  ConstraintSet constraints_fZoneOnly;
  ...
  /// @brief Ordered list of symbolic arrays: used to generate test cases.
  //
  // FIXME: Move to a shared list structure (not critical).
  std::vector<std::pair<ref<const MemoryObject>, const Array *>> symbolic_arrays;

  /// @brief Ordered list of symbolic bitvectors: used to generate test cases.
  std::vector<ref<BitVecExpr>> symbolic_bitvecs;

  std::vector<ref<IntExpr>> symbolic_ints;

  // TODO: do the adjustments you did for symbolic bitvecs for SMT functions as well.

  syt_execution_stage syt_stage;


  std::vector<InvHavocLevel> activeInvariants;
  

  // We need to keep track of constraints added in a lambda path.
  // The current lambda path has added constaints in lambdaPathConstraints.back()
  // We maintain a nested vector to handle lambdas calling lambdas.
  std::vector<std::vector<ref<Expr>>> lambdaPathConstraints;

  // If *this is a lambda state, this is a pointer to the caller state.
  ExecutionState *lambdaParent = NULL;
  bool nameParentsObjects = false;
  llvm::Function *lambdaFunction = NULL;
  /* instantiating fzone elements is not safe if the purpose is to memoize*/
  enum LambdaPurpose lambdaPurpose = MEMOIZE;

  std::string forkReason = "";

  std::vector<std::pair<KInstruction *, std::string>> multipleResHistory;

  std::map<ForallElemProperty *, std::vector<ref<Expr>>> instantiatedFEPs;

  // This is the flag flipped by syt_switch_old / syt_switch_new
  bool computeOldValues = false;

  std::set<ref<Expr>> knownUnsignedExprs;
  ...
  bool hasMadeSmokeTest = false;

  bool skipInvariantChecks = false;

  std::vector<KInstruction*> instHistory;

  bool isModified = false;

  std::set<const MemoryObject *> modifiedGlobals;
  std::map<llvm::Function *, std::set<const MemoryObject *>> invDependencies;
  std::map<llvm::Function *, std::vector<std::pair<const MemoryObject *, std::string>>> invNamings;
  llvm::Function *invBeingAssumed = nullptr;
...
public:
  ...
  void addSymbolic(ref<BitVecExpr> bv);
  void addSymbolic(ref<IntExpr> ie);
  ...
  ConstraintSet getConstraints(bool useHeapConstraints, bool useFZoneConstraints=false, int fZoneIdx=-1) const {
    // ConstraintSet ret_constraints = constraints;
    assert(constraints_heapOnly.constraints.size() == 0); // we don't use heapOnly constraints anymore.

    // if (useHeapConstraints) {
    //   ret_constraints = ret_constraints.join(constraints_heapOnly);

    // }
    
    if (useFZoneConstraints) {
      assert(fZoneIdx >= 0);
      // Add all fzone constraints
      // ret_constraints = ret_constraints.join(constraints_fZoneOnly);

      // Add a specific fzone constraint
      std::vector<ref<Expr>> singletonVector = {constraints_fZoneOnly.constraints[fZoneIdx]};
      ConstraintSet singleton(singletonVector);

      ConstraintSet res = constraints.join(singleton);
      res.readSimplCache = constraints.readSimplCache;
      res.readSimplKnownFailures = constraints.readSimplKnownFailures;
      res.addressSimplTable = constraints.addressSimplTable;
      return res;
    }
    return constraints;
  }
  ...
  unsigned int curLine() const;
  void dumpMultiResHistory() const;

  ref<Expr> getUnsignedConstraints(std::vector<ref<Expr>> unsignedExprs);
