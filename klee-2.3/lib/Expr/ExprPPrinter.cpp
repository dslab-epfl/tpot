...
private:
  ...
  const ReadExpr* hasOrderedReads(ref<Expr> e, int stride) {
    ...
    // Let's mot worry about concat chains over integer-indexed arrays for now.
    if (base->index->getWidth() == 0)
      return NULL;
  ...
  void printBitVec(const BitVecExpr *be, PrintContext &PC, unsigned indent) {
    PC << be->name;
  }

  void printInt(const IntExpr *ie, PrintContext &PC, unsigned indent) {
    PC << ie->name;
  }
...
public:
  ...
  void print(const ref<Expr> &e, PrintContext &PC, bool printConstWidth=false) {
	...
	PC << '(' << e->getKind();
        ...
        } else if (e->getKind() == Expr::BitVec) {
          // for some reason using dyn_cast directly picks up Forall expressions and crashes.
          const BitVecExpr *be = dyn_cast<BitVecExpr>(e);
          assert(be != NULL && "dyn_cast failed");
          printBitVec(be, PC, indent);
        } else if (e->getKind() == Expr::Int) {
          const IntExpr *ie = dyn_cast<IntExpr>(e);
          assert(ie != NULL && "dyn_cast failed");
          printInt(ie, PC, indent);
