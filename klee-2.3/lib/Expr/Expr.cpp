...
#include "klee/Expr/ExprVisitor.h" // for ExprVisitor
...
ref<ConstantExpr> ConstantExpr::Slt(const ref<ConstantExpr> &RHS) {
  ...
  return ConstantExpr::alloc(value.getSExtValue() < RHS->value.getSExtValue(), Expr::Bool);
}
...
// If one of _l and _r is an integer and the other one is a constant,
// converts the constant into an integer and returns true.
static bool checkIntOp(const ref<Expr> &_l, const ref<Expr> &_r, 
                       ref<Expr> &l, ref<Expr> &r) {
  if (_l->getWidth() == _r->getWidth()) {
    l = _l; r = _r;
    if (l->getWidth() == 0) {
      // Returning true prevents further optimization. 
      // We want this for integers. Otherwise we run into subtle bugs
      // related to e.g. optimization of sub expresssions. (see SubExpr_createPartial)
      return true;
    }
    return false;
  }
  // width == 0 represents a Z3 integer
  if (_l->getWidth() == 0 && dyn_cast<ConstantExpr>(_r)) {
    l = _l; 
    ref<ConstantExpr> r_const = ConstantExpr::alloc(cast<ConstantExpr>(_r)->value);
    r_const->producesInt = true;
    r = r_const;
    return true;
  }
  if (_r->getWidth() == 0 && dyn_cast<ConstantExpr>(_l)) {
    r = _r; 
    ref<ConstantExpr> l_const = ConstantExpr::alloc(cast<ConstantExpr>(_l)->value);
    l_const->producesInt = true;
    l = l_const;
    return true;
  }
...
ref<Expr> ReadExpr::create(const UpdateList &ul, ref<Expr> index) {
  if (ul.root) {
    assert(ul.root->getDomain() == index->getWidth());
  }
...
ref<Expr> SelectExpr::create(ref<Expr> c, ref<Expr> _t, ref<Expr> _f) {
  ref<Expr> t, f;                                                   \
  checkIntOp(_t, _f, t, f);
...
ref<Expr> ZExtExpr::create(const ref<Expr> &e, Width w) {
  ...
  // assert(kBits != 0 && "Trying to zext an integer expression");
// #ifndef NDEBUG
//   if (kBits == 0)
//     raise(SIGINT);
// #endif
  ...
  if (type == Expr::Bool) {
    ...
    if (lk==Expr::Add && isa<ConstantExpr>(l->getKid(0))) { // (k+a)+b = k+(a+b)
      ...
      return AddExpr::create(l->getKid(0),
        ...
        type != 0 /* operands of int additions may noy be canonicalized and give
                     rise to sublte infinite recursion issues. */
      ) {
...
static ref<ConstantExpr> allocConstZero(Expr::Width type) {
  if (type == 0) {
      // This is a Z3 integer. Allocating CE with width 0 will
      // cause an error. Let's choose width 64. Shouldn't matter.
      ref<ConstantExpr> res = ConstantExpr::alloc(0, 64);
...
static ref<Expr> SubExpr_createPartial(Expr *l, const ref<ConstantExpr> &cr) {
  // l - c => l + (-c)
  return AddExpr_createPartial(l, allocConstZero(cr->getWidth())->Sub(cr));
}
static ref<Expr> SubExpr_create(Expr *l, Expr *r) {
  ...
  if (type == Expr::Bool) {
    ...
    return allocConstZero(type);
  } else if (CE && CE->isZero()) {
      return l;
  } else {
    Expr::Kind lk = l->getKind(), rk = r->getKind();
    if (lk==Expr::Add && isa<ConstantExpr>(l->getKid(0))) { // (k+a)-b = k+(a-b)
      ...
      return SubExpr::create(AddExpr::create(l, r->getKid(1)),
                             r->getKid(0));
    // } else if (lk==Expr::Add && isa<ConstantExpr>(l->getKid(1))) {
    //   // Klee does not require the symmetric cases below but we do since we
    //   // do not exactly follow the canonical form...
    //   return AddExpr::create(l->getKid(1),
    //                          SubExpr::create(l->getKid(0), r));
    // } else if (rk==Expr::Add && isa<ConstantExpr>(r->getKid(1))) { 
    //   return SubExpr::create(SubExpr::create(l, r->getKid(0)),
    //                          r->getKid(1));
    } else {
...
ref<Expr>  _e_op ::create(const ref<Expr> &_l, const ref<Expr> &_r) {   \
  ref<Expr> l, r;                                                       \
  if (checkIntOp(_l, _r, l, r)) {                                       \
    if (ConstantExpr *cl = dyn_cast<ConstantExpr>(l))                   \
      if (ConstantExpr *cr = dyn_cast<ConstantExpr>(r)) {               \
        ref<ConstantExpr> res = cl->_op(cr);                            \
        assert(cl->producesInt == cr->producesInt);                     \
        res->producesInt = cl->producesInt;                             \
        return res;                                                     \
      }                                                                 \
  ...
  if (checkIntOp(_l, _r, l, r)) {                                   \
    if (ConstantExpr *cl = dyn_cast<ConstantExpr>(l))                   \
      if (ConstantExpr *cr = dyn_cast<ConstantExpr>(r)) {               \
        ref<ConstantExpr> res = cl->_op(cr);                            \
        assert(cl->producesInt == cr->producesInt);                     \
        /*comparison never produces an int*/                            \
        /*res->producesInt = cl->producesInt;*/                         \
        return res;                                                     \
      }                                                                 \
    return _e_op ## _create(l, r);                                  \
...
std::vector<struct FuncDecl> klee::smtFuncs;

void klee::initSMTFuncs() {
  struct FuncDecl is_unique;
  is_unique.name =  "is_unique";
  // We experimentally represent heap pointers with Z3 integers
  // is_unique.range = 64; // Expr::Int64  .. this throws a linker error for some reason
  // is_unique.argTypes.push_back(64); // Expr::Int6
  is_unique.range = 0; 
  is_unique.argTypes.push_back(0);
  smtFuncs.push_back(is_unique);


  struct FuncDecl tpot_bv2int;
  tpot_bv2int.name = "tpot_bv2int";
  tpot_bv2int.range = 0;
  tpot_bv2int.argTypes.push_back(64);
  smtFuncs.push_back(tpot_bv2int);  
}

void klee::addSMTFunc(std::string name, 
    std::vector<Expr::Width> argTypes, Expr::Width range_width) {
  for (struct FuncDecl f : smtFuncs) {
    if (f.name == name) {
      return;
    }
  }
  ...
  f.range = range_width;
  for (int i : argTypes) {
    f.argTypes.push_back(i);
  }
  smtFuncs.push_back(f);  
}


#define _ATOM_CASE(op)                    \
  case Expr:: op : {                      \
    return BV2IntExpr::create(expr);      \
  }                   

#define _BINOP_CASE(op) \
  case Expr:: op : {                                                  \
    op ## Expr *ae = cast<op ## Expr>(expr);                          \
    ref<Expr> left_int = convert(ae->left, unsignedExprs);            \
    ref<Expr> right_int = convert(ae->right, unsignedExprs);          \
    return op ## Expr::create(left_int, right_int);                   \
  }

static ref<Expr> intLeftShiftConst(ref<Expr> i, uint64_t order) {
  // multiplication by power of two
  uint64_t coeff = 1 << order;
  ref<Expr> coeffExpr = ConstantExpr::create(coeff, 64);
  return MulExpr::create(i, coeffExpr);
}

std::map<std::pair<ref<Expr>, ref<Expr>>, ref<Expr>> intLeftShiftCache;


static ref<Expr> intLeftShift(ref<Expr> i, ref<Expr> order) {
  static int idx = 0;
  if (ConstantExpr *order_c = dyn_cast<ConstantExpr>(order)) {
    return intLeftShiftConst(i, order_c->getZExtValue());
  }

  if (intLeftShiftCache.find(std::make_pair(i, order)) != intLeftShiftCache.end()) {
    return intLeftShiftCache[std::make_pair(i, order)];
  }

  ref<Expr> baseCase = IntExpr::create("leftShiftBaseCase" + std::to_string(idx++));

  ref<Expr> res = baseCase;

  for (uint64_t j = 0; j < 23; j++) {
    ref<Expr> constCase = ConstantExpr::create(j, 64);
    res = SelectExpr::create(
      EqExpr::create(order, constCase),
      intLeftShiftConst(i, j),
      res);
  }

  intLeftShiftCache[std::make_pair(i, order)] = res;

  return res;
}

ref<Expr> BV2IntExpr::convert(ref<Expr> expr, std::vector<ref<Expr>> &unsignedExprs) {
  // Now that we ignore zero extension, we can't assert this anymore.
  ...
  // -- Atoms -- //
  // We have no axioms on how atoms are converted.
  _ATOM_CASE(BitVec)
  // _ATOM_CASE(SExt)
  _ATOM_CASE(Concat)
  _ATOM_CASE(Read)
  ...
  _ATOM_CASE(And) // We'll treat this as an atom for now and
                  // add axioms for bitwise operations later.

  // -- Binary Operations -- //
  // Base axiom: BV2Int(a op b) = BV2Int(a) op BV2Int(b)
  // Recursively generate axioms for BV2Int(a) and BV2Int(b), too.
  _BINOP_CASE(Add)
  _BINOP_CASE(Sub)
  _BINOP_CASE(Mul)
  _BINOP_CASE(SDiv)
  ...
  // these might be trouble
  _BINOP_CASE(URem)
  _BINOP_CASE(UDiv)

  // -- Other -- //
  // _ATOM_CASE(Constant) // Constants get converted to themselves
  case Expr::Constant: {
  ...
  case Expr::Select: {
    ...
    return SelectExpr::create(convertConstraint(se->cond, unsignedExprs), 
      convert(se->trueExpr, unsignedExprs), convert(se->falseExpr, unsignedExprs));
  }

  case Expr::Shl: {
    ShlExpr *se = cast<ShlExpr>(expr);
    return intLeftShift(convert(se->left, unsignedExprs), convert(se->right, unsignedExprs));
  }

  case Expr::LShr: {
    LShrExpr *se = cast<LShrExpr>(expr);
    if (ConstantExpr *right_c = dyn_cast<ConstantExpr>(se->right)) {
      // division by power of two
      uint64_t coeff = 1 << right_c->getZExtValue();
      ref<Expr> coeffExpr = ConstantExpr::create(coeff, 64);
      ref<Expr> leftInt = convert(se->left, unsignedExprs);
      return SDivExpr::create(leftInt, coeffExpr);
    } else {
      assert(0 && "tpot-bv2int can not handle non-constant shift amount for now");
    }
  }

  case Expr::ZExt: {
    // Ignore zero extension, cross fingers.
    // The BV2Int constructor will zero extend atoms.
    ZExtExpr *ze = cast<ZExtExpr>(expr);
    ref<Expr> srcInt = convert(ze->src, unsignedExprs);
    return srcInt;
  }

  //! Experimental...... 
  case Expr::SExt: {
    // Ignore sign extension, cross fingers.
    // The BV2Int constructor will zero extend atoms.
    SExtExpr *se = cast<SExtExpr>(expr);
    ref<Expr> srcInt = convert(se->src, unsignedExprs);
    return srcInt;
  }

  case Expr::Xor: {
    XorExpr *xe = cast<XorExpr>(expr);
    ref<Expr> leftInt = convert(xe->left, unsignedExprs);

    // Special case: i xor ((1 << p) << o) == i + 2^(o + p) * (1 - 2*((i / 2^(o+p)) % 2))
    if (ShlExpr *se = dyn_cast<ShlExpr>(xe->right)) {
      if (ConstantExpr *left_c = dyn_cast<ConstantExpr>(se->left)) {
        uint64_t n = left_c->getZExtValue();
        // is n a power of two?
        if (n && !(n & (n - 1))) {
          int p = 0;
          while (n >>= 1) ++p;

          ref<Expr> oInt = convert(se->right, unsignedExprs);

          // compute 2^(o+p)
          ref<Expr> exp = intLeftShift(ConstantExpr::create(1, 64),
                            AddExpr::create(oInt, ConstantExpr::create(p, 64)));

          ref<Expr> i = leftInt;
          return AddExpr::create(i, 
            MulExpr::create(exp,
              SubExpr::create(ConstantExpr::create(1, 64),
                MulExpr::create(ConstantExpr::create(2, 64),
                  URemExpr::create(SDivExpr::create(i, exp),
                    ConstantExpr::create(2, 64)))))); 
        }
      }
    }
    // TODO: make this symmetric between left and right arguments

    // Same as above, but with a constant shift amount
    if (ConstantExpr *ce = dyn_cast<ConstantExpr>(xe->left)) {
      uint64_t n = ce->getZExtValue();
      // is n a power of two?
      if (n && !(n & (n - 1))) {
        ref<Expr> exp = ce;
  ...
  case Expr::Or: {
    // let's treat this as an atom for now.
    OrExpr *se = cast<OrExpr>(expr);
    return BV2IntExpr::create(se);;
  }
...
#define GETBVFORM_BINOP_CASE(op) \
  case Expr:: op : {                                                 \
    op ## Expr *ae = cast<op ## Expr>(expr);                         \
    ref<Expr> left_bv= getBVForm(ae->left);                          \
    ref<Expr> right_bv = getBVForm(ae->right);                       \
    return op ## Expr::create(left_bv, right_bv);                   \
  }

ref<Expr> BV2IntExpr::getBVForm(const ref<Expr> &expr) {
  assert(expr->getWidth() == 0);
  ...
  switch (expr->getKind()) {
    GETBVFORM_BINOP_CASE(Add)
    GETBVFORM_BINOP_CASE(Sub)
    GETBVFORM_BINOP_CASE(Mul)
    GETBVFORM_BINOP_CASE(SDiv)
    GETBVFORM_BINOP_CASE(SRem)

    GETBVFORM_BINOP_CASE(UDiv)
    GETBVFORM_BINOP_CASE(URem)

    case Expr::BV2Int: {
      BV2IntExpr *b2i = cast<BV2IntExpr>(expr);
      return b2i->expr;
    }
    
    case Expr::Constant: {
      ConstantExpr *ce = cast<ConstantExpr>(expr);
      return ConstantExpr::alloc(ce->getZExtValue(64), Expr::Int64);
    }

    case Expr::Select: {
    SelectExpr *se = cast<SelectExpr>(expr);
    //? Do we need to convert the condition to BV?
    return SelectExpr::create(se->cond, getBVForm(se->trueExpr), getBVForm(se->falseExpr));
  }
...
ref<Expr> BV2IntExpr::convertConstraint(const ref<Expr> &e, std::vector<ref<Expr>> &unsignedExprs) {
  if (e->getKind() == Expr::Constant) {
    return e;
  }

  assert(e->getWidth() == 1 && "only propositional formulas are supported");

  // We consider Extract expressions as atoms, too.
  // Except when they represent booleans.
  if (e->getKind() == Expr::Extract) {
    ExtractExpr *ee = cast<ExtractExpr>(e);
    if (ee->expr->getKind() == Expr::ZExt &&
        ee->expr->getKid(0)->getWidth() == 1) {
      return convertConstraint(ee->expr->getKid(0), unsignedExprs);
    } else if (ee->offset == 0 && e->getWidth() == 1) {
      // (Extract 0 (BitVec w8 ...))
      return convertConstraint(EqExpr_create(
          ConstantExpr::create(1, 64),
          URemExpr_create(ee->expr, ConstantExpr::create(2, 64))), unsignedExprs);
    }
  }

  // Treat quantified expressions differently. Since the bound variable can't be
  // an application of bv2int, we need to use subsitutions in the body.
  if (e->getKind() == Expr::Forall) {
    // convert bound variable (i) to an integer. In the body,
    // replace bv2int(i) with the integer value.
    ForallExpr *fe = cast<ForallExpr>(e);
    BitVecExpr *var = cast<BitVecExpr>(fe->var);
    if (var->getWidth() == 0) {
      // this is already a formula over integers
      return e;
    }

    ref<Expr> var_int = IntExpr::create(var->name + "_int");

    auto body_int = convertConstraint(fe->body, unsignedExprs);

    ExprReplaceVisitor erv(BV2IntExpr::create(var), var_int);
    auto body_int_subst = erv.visit(body_int);

    auto res = ForallExpr::create(var_int, body_int_subst);

    return res;
  }

  if (e->getKind() == Expr::Ult) {
    UltExpr *ult = cast<UltExpr>(e);
    ref<Expr> leftInt = ult->left;
    if (!isa<ConstantExpr>(leftInt) && leftInt->getWidth() > 0) {
      leftInt = convert(ult->left, unsignedExprs);
      unsignedExprs.push_back(leftInt);
    }
    ref<Expr> rightInt = ult->right;
    if (!isa<ConstantExpr>(rightInt) && rightInt->getWidth() > 0) {
      rightInt = convert(ult->right, unsignedExprs);
      unsignedExprs.push_back(rightInt);
    }
    return  UltExpr::create(leftInt, rightInt);
    // The bv2int conversion is unsigned. Add the conjunct that left is positive 
    // if right is positive
    // return AndExpr::create(
    //   createImplies(UleExpr::create(ConstantExpr::create(0, 64), rightInt),
    //                 UleExpr::create(ConstantExpr::create(0, 64), leftInt)),
    //   UltExpr::create(leftInt, rightInt));
  }

  if (e->getKind() == Expr::Ule) {
    UleExpr *ule = cast<UleExpr>(e);
    ref<Expr> leftInt = ule->left;
    if (!isa<ConstantExpr>(leftInt) && leftInt->getWidth() > 0) {
      leftInt = convert(ule->left, unsignedExprs);
      unsignedExprs.push_back(leftInt);
    }
    ref<Expr> rightInt = ule->right;
    if (!isa<ConstantExpr>(rightInt) && rightInt->getWidth() > 0) {
      rightInt = convert(ule->right, unsignedExprs);
      unsignedExprs.push_back(rightInt);
    }
    return  UleExpr::create(leftInt, rightInt);
    // The bv2int conversion is unsigned. Add the conjunct that left is positive 
    // if right is positive
    // return AndExpr::create(
    //   createImplies(UleExpr::create(ConstantExpr::create(0, 64), rightInt),
    //                 UleExpr::create(ConstantExpr::create(0, 64), leftInt)),
    //   UleExpr::create(leftInt, rightInt));
  }

    if (e->getKind() == Expr::Ugt) {
    UgeExpr *ugt = cast<UgeExpr>(e);
    ref<Expr> leftInt = ugt->left;
    if (!isa<ConstantExpr>(leftInt) && leftInt->getWidth() > 0) {
      leftInt = convert(ugt->left, unsignedExprs);
      unsignedExprs.push_back(leftInt);
    }
    ref<Expr> rightInt = ugt->right;
    if (!isa<ConstantExpr>(rightInt) && rightInt->getWidth() > 0) {
      rightInt = convert(ugt->right, unsignedExprs);
      unsignedExprs.push_back(rightInt);
    }
    return  UgtExpr::create(leftInt, rightInt);
    // return AndExpr::create(
    //   createImplies(UleExpr::create(ConstantExpr::create(0, 64), leftInt),
    //                 UleExpr::create(ConstantExpr::create(0, 64), rightInt)),
    //   UgtExpr::create(leftInt, rightInt));
  }

  if (e->getKind() == Expr::Uge) {
    UgeExpr *uge = cast<UgeExpr>(e);
    ref<Expr> leftInt = uge->left;
    if (!isa<ConstantExpr>(leftInt) && leftInt->getWidth() > 0) {
      leftInt = convert(uge->left, unsignedExprs);
      unsignedExprs.push_back(leftInt);
    }
    ref<Expr> rightInt = uge->right;
    if (!isa<ConstantExpr>(rightInt) && rightInt->getWidth() > 0) {
      rightInt = convert(uge->right, unsignedExprs);
      unsignedExprs.push_back(rightInt);
    }
    return  UgeExpr::create(leftInt, rightInt);
    // return AndExpr::create(
    //   createImplies(UleExpr::create(ConstantExpr::create(0, 64), leftInt),
    //                 UleExpr::create(ConstantExpr::create(0, 64), rightInt)),
    //   UgeExpr::create(leftInt, rightInt));
  }


  //  -- general case --
  // there is no kind of expression with >= 10 kids, right?
  assert(e->getNumKids() < 10 && "too many kids");
  ref<Expr> kids[10];
  for (int i = 0; i < e->getNumKids(); i++) {
    ref<Expr> kid = e->getKid(i);
    if (kid->getWidth() == 1) {
      // recurse into propositional subformulas
      kids[i] = convertConstraint(kid, unsignedExprs);
    } else if (kid->getWidth() == 0) {
      kids[i] = kid;
    } else {
      if (kid->getWidth() == 64) {
        kids[i] = BV2IntExpr::convert(kid, unsignedExprs);
      } else {
        assert(kid->getWidth() < 64 && kid->getWidth() > 0 && "too wide");
        // kids[i] = BV2IntExpr::convert(ConcatExpr::create(
        //     ConstantExpr::create(0, 64 - kid->getWidth()), kid), unsignedExprs);
        if (kid->getKind() == Expr::Constant) {
          // Don't zext constants. They might be signed.
          kids[i] = BV2IntExpr::convert(kid, unsignedExprs);
        } else {
          kids[i] = BV2IntExpr::convert(SExtExpr::create(kid, 64), unsignedExprs);
        }
      }
    }
  }
  ...
  // ---
  //   ref<Expr> acc = ConstantExpr::create(1, 1);

  // ref<Expr> extraConstr1 = NULL;
  // ref<Expr> extraConstr2 = NULL;

  // for (; i < e->getNumKids(); i++) {
  //   ref<Expr> kid = e->getKid(i);
  //   if (kid->getWidth() == 1) {
  //     // recurse into propositional subformulas
  //     kids[i] = getBV2IntConstraint(kid);
  //   } else if (kid->getWidth() == 0) {
  //     kids[i] = kid;
  //   } else {
  //     if (kid->getWidth() == 64) {
  //       kids[i] = BV2IntExpr::create(kid, extraConstr1);
  //       if (!extraConstr1.isNull()) {
  //         acc = AndExpr::create(acc, extraConstr1);
  //       }
  //     } else {
  //       assert(kid->getWidth() < 64 && kid->getWidth() > 0 && "too wide");
  //       kids[i] = BV2IntExpr::create(ConcatExpr::create(
  //           ConstantExpr::create(0, 64 - kid->getWidth()), kid), extraConstr2);
  //       if (!extraConstr2.isNull()) {
  //         acc = AndExpr::create(acc, extraConstr2);
  //       }
  //     }
  //   }
  // }
  ...
  // auto res = e->rebuild(kids);
  // assert(res->getWidth() == 1);
  // return AndExpr::create(acc, res);
}

unsigned RealInt2BVExpr::computeHash() {
  hashValue = expr->hash() * Expr::MAGIC_HASH_CONSTANT * Expr::RealInt2BV;
  return hashValue;
}

ref<Expr> RealInt2BVExpr::create(const ref<Expr> &e) {
  if (BV2IntExpr *bv2int = dyn_cast<BV2IntExpr>(e))
    return RealInt2BVExpr::alloc(e);

  assert(false && "Hey, what happened?");
}

void RealInt2BVExpr::convertConstraint(
    const ref<Expr> &e, std::vector<ref<BV2IntExpr>> &tpot_bv2int_exprs) {
  if (e->getNumKids() < 1) {
    return;
  }
  if (e->getKind() == Expr::BV2Int) {
    tpot_bv2int_exprs.push_back(cast<BV2IntExpr>(e));
  }
  for (size_t i = 0; i < e->getNumKids(); i++) {
    ref<Expr> kid = e->getKid(i);
    convertConstraint(kid, tpot_bv2int_exprs);
  }
}