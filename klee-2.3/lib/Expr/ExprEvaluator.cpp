...
ExprVisitor::Action ExprEvaluator::visitExpr(const Expr &e) {
  ...
  if (!N || isa<ReadExpr>(e) || isa<BitVecExpr>(e))
...
ExprVisitor::Action ExprEvaluator::visitBitVec(const BitVecExpr &bv) {
  return Action::changeTo(getInitialValue(bv));
}

ExprVisitor::Action ExprEvaluator::visitInt(const IntExpr &ie) {
  return Action::changeTo(getInitialValue(ie));
}
