...
bool 
StagedSolverImpl::computeInitialValues(
    const Query &query,
    ...
    bool &hasSolution) {
  if (primary->computeInitialValues(query, arr_objects, arr_values,
       bv_objects, bv_values, 
       int_objects, int_values, hasSolution))
    return true;
  
  return secondary->impl->computeInitialValues(query, arr_objects, arr_values,
                                               bv_objects, bv_values,
