...
bool Solver::getCounterExample(
  ...
  bool hasSolution;
  bool success =
    impl->computeCounterExample(query, arr_objects, arr_values, arr_values_ok, 
                          bv_objects, bv_values, bv_values_ok, int_objects, int_values,
                          int_values_ok, hasSolution);
...
bool Solver::getValue(const Query& query, ref<ConstantExpr> &result) {
  ...
  klee_error("TPot failure: Trying to concretize non-constant expression");
...
Solver::getInitialValues(const Query &query,
  ...
  bool hasSolution;
  bool success =
    impl->computeInitialValues(query, arr_objects, arr_values, 
