...
protected:
  ...
  ref<Expr> getInitialValue(const BitVecExpr &bv) {
    //! This should not be used. Disable the fastCexSolver from the command line.
    klee_error("SyT using FastCexSolver.. This is not safe for now");
    return ConstantExpr::alloc(19, bv.getWidth());
  }

  ref<Expr> getInitialValue(const IntExpr &ie) {
    //! This should not be used. Disable the fastCexSolver from the command line.
    klee_error("SyT using FastCexSolver.. This is not safe for now");
    return ConstantExpr::alloc(19, ie.getWidth());
  }
