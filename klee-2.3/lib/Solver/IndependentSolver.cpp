...
bool assertCreatedPointEvaluatesToTrue(
  ...
  std::vector<const BitVecExpr *> bitvecs;
  std::vector<bitvec_ce_t> bv_values;
  std::vector<const IntExpr *> ints;
  std::vector<int_ce_t> int_values;
  Assignment assign = Assignment(objects, values, bitvecs, bv_values, ints, int_values, /*_allowFreeValues=*/true);
  ...
  if (retMap.size() > 0)
    assign.arr_bindings.insert(retMap.begin(), retMap.end());
  //TODO: should bitvec bindings be handled?

  for (auto const &constraint : query.constraints) {
    ref<Expr> ret = assign.evaluate(constraint);
    Expr::Kind kind = ret->getKind();
    if (kind == Expr::Forall || kind == Expr::Exists || kind == Expr::BitVec)
       return true;
...
bool IndependentSolver::computeInitialValues(
  ...
  assert(0 && "Syt does not support IndependentSolver yet");
  ...
  for (std::list<IndependentElementSet>::iterator it = factors->begin();
    ...
    std::vector<const BitVecExpr *> dummyBVObjects;
    std::vector<bitvec_ce_t> dummyBVValues;
    std::vector<const IntExpr *> dummyIntObjects;
    std::vector<int_ce_t> dummyIntValues;

    if (!solver->impl->computeInitialValues(Query(tmp, ConstantExpr::alloc(0, Expr::Bool)),
                                            arraysInFactor, tempValues,
                                            dummyBVObjects, dummyBVValues,
                                            dummyIntObjects, dummyIntValues, hasSolution)){
      arr_values.clear();
      delete factors;
      return false;
    } else if (!hasSolution){
  ...
  for (std::vector<const Array *>::const_iterator it = arr_objects.begin();
    ...
    if (!retMap.count(arr)){
      ...
      arr_values.push_back(ret);
    } else {
      arr_values.push_back(retMap[arr]);
    }
  }
  assert(assertCreatedPointEvaluatesToTrue(query, arr_objects, arr_values, retMap) && "should satisfy the equation");
  delete factors;
  return true;
}
