...
class Z3Builder {
  ...
  std::unordered_map<std::string, Z3ASTHandle> constructed_bitvecs;
  std::unordered_map<std::string, Z3ASTHandle> constructed_ints;
...
private:
  ...
  // Integer comparison
  Z3ASTHandle intLtExpr(Z3ASTHandle lhs, Z3ASTHandle rhs);
  Z3ASTHandle intLeExpr(Z3ASTHandle lhs, Z3ASTHandle rhs);

  // these also imply that lhs and rhs are positive.
  // Z3ASTHandle intUltExpr(Z3ASTHandle lhs, Z3ASTHandle rhs);
  // Z3ASTHandle intUleExpr(Z3ASTHandle lhs, Z3ASTHandle rhs);
  ...
  Z3ASTHandle bitVecExpr(Z3_symbol symb, Z3_sort bv_sort);
  Z3ASTHandle arrayEqExpr(Z3ASTHandle a, Z3ASTHandle b);
  Z3ASTHandle funcAppExpr(Z3_func_decl f, std::vector<Z3ASTHandle> args);
  Z3ASTHandle bv2IntExpr(Z3ASTHandle bv);

  Z3ASTHandle realInt2BVExpr(Z3ASTHandle integer);

  klee::Z3SortHandle getArgumentSort(unsigned width);
...
public:
  ...
  // Returns a handle for a symbolic bitvector previously constructed with the same name.
  Z3ASTHandle getBVHandle(std::string name, Expr::Width width);

  // Returns a handle for a symbolic int previously constructed with the same name.
  Z3ASTHandle getIntHandle(std::string name);

  Z3ASTHandle intExpr(Z3_symbol symb);
