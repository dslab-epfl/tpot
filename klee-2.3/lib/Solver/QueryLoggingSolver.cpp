...
void QueryLoggingSolver::startQuery(const Query &query, const char *typeName,
  ...
  printQuery(query, falseQuery, arr_objects, bv_objects);
...
bool QueryLoggingSolver::computeInitialValues(
    ...
    bool &hasSolution){
  startQuery(query, "InitialValues", 0, &arr_objects, &bv_objects);

  bool success =
      solver->impl->computeInitialValues(query, arr_objects, arr_values,
  ...
  if (success) {
    ...
    if (hasSolution) {
      ...
      for (std::vector<const Array *>::const_iterator i = arr_objects.begin(),
                                                      e = arr_objects.end();
           i != e; ++i, ++values_it) {
        const Array *array = *i;
