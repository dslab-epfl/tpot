...
void KModule::manifest(InterpreterHandler *ih, bool forceSourceOutput) {
  ...
  for (auto &Function : *module) {
    if (Function.isDeclaration()) {
      if (Function.getName().startswith("uninterp__")) {
        uninterpFunctions.push_back(&Function);
      }
    ...
    if (Function.getName().startswith("inv__")) {
      if (!Function.getReturnType()->isIntegerTy(1))
        klee_error("Invariant function %s must have boolean return type", 
                   Function.getName().str().c_str());

      sytInvariants.push_back(&Function);
    }

    if (Function.getName().startswith("inv_unchecked__")) {
      if (!Function.getReturnType()->isIntegerTy(1))
        klee_error("Invariant function %s must have boolean return type", 
                   Function.getName().str().c_str());

      sytUncheckedInvariants.push_back(&Function);
    }

    if (Function.getName().startswith("uninterp__")) {
      klee_error("uninterp__ functions should not be defined");
    }
  }
