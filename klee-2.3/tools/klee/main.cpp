...
namespace {
  ...
  cl::opt<std::string>
  SytInitConstraints("syt-init-constraints",
             cl::desc("Pass an smt file specifying constraints on init state"),
             cl::init(""),
             cl::cat(SytCat));

  cl::opt<std::string>
  TPotConfigFile("tpot-config",
             cl::desc("Pass a tpot config file"),
             cl::init("tpot_config"),
             cl::cat(SytCat));

  cl::opt<std::string>
  SytPostConstraints("syt-post-constraints",
             cl::desc("Specify an smt file to dump constraints on post-state"),
             cl::init(""),
             cl::cat(SytCat));
  ...
  SmokeTest("smoke-test",
           cl::desc("Run smoke tests"),
           cl::init(false),
           cl::cat(SytCat));
  cl::opt<unsigned>
  MaxSmokeTestCount("max-smoke-test-count",
           cl::desc("Number of smoke tests to run"),
           cl::init(1),
           cl::cat(SytCat));

  cl::list<std::string>
  APIFunctions("api-functions",
               cl::desc("Specify API functions"),
               cl::value_desc("API functions"),
               cl::cat(SytCat));
}
...
private:
  ...
  unsigned m_errorPaths;  // number of paths that lead to errors
  unsigned m_leakyPaths;  // number of paths that leak memory
...
public:
  ...
  unsigned getNumErrorPaths() { return m_errorPaths; }
  unsigned getNumLeakyPaths() { return m_leakyPaths; }
  void incPathsCompleted() { ++m_pathsCompleted; }
  void incPathsExplored(std::uint32_t num = 1) {
    m_pathsExplored += num; }
  void incErrorPaths() { ++m_errorPaths; }
  void incLeakyPaths() { ++m_leakyPaths; }
...
KleeHandler::KleeHandler(int argc, char **argv)
    : m_interpreter(0), m_pathWriter(0), m_symPathWriter(0),
      ...
      m_pathsCompleted(0), m_pathsExplored(0), m_errorPaths(0), m_leakyPaths(0), m_argc(argc), m_argv(argv) {
...
void KleeHandler::processTestCase(const ExecutionState &state,
  ...
  if (!WriteNone) {
    ...
    if (m_numGeneratedTests > 0 && m_numGeneratedTests == MaxTests)
...
static const char *modelledExternals[] = {
  ...
  "__tpot_fresh_bv", 
  "__tpot_points_to",
  "__tpot_names_obj_forall",
  "__tpot_names_obj_forall_cond",
  "__tpot_inv",
  "__tpot_forall_elem",
  "__tpot_assert",
  "__tpot_switch_old",
  "__tpot_switch_new",
  "__tpot_uninterpreted",
  "__tpot_assert_unchanged_except",
  "__tpot_malloc",
  "__tpot_free",
  "__tpot_debug_print",

  // TODO: warn when this is used.
  "__tpot_skip_invariant_check"
};
...
static void interrupt_handle() {
  if (!interrupted && theInterpreter) {
    ...
    // TPot currently has a bunch of raise(SIGINT)'s where execution fails. Let's make sure 
    // cause the CI to fail.
    // TODO: remove this once we have a better way to handle it.
    exit(1); 
  } else {
...
int main(int argc, char **argv, char **envp) {
  ...
  Interpreter *interpreter =
    theInterpreter = Interpreter::create(ctx, IOpts, handler, SytInitConstraints);
  assert(interpreter);

  if (SytPostConstraints != "") {
    interpreter->setSytPostConstraints(SytPostConstraints);
  }
  ...
  interpreter->initializeSolver(SytInitConstraints);

  interpreter->readTPotConfig(TPotConfigFile);
  ...
  interpreter->applyInvariantRestrictions(mainFn->getName().str());
  ...
  if (SmokeTest) {
    interpreter->setSmokeTest(SmokeTest);
    interpreter->setMaxSmokeTestCount(MaxSmokeTestCount);
  }

  if (!APIFunctions.empty()) {
    interpreter->setAPIFunctions(APIFunctions);
  }
  ...
  if (!ReplayKTestDir.empty() || !ReplayKTestFile.empty()) {
    ...
    interpreter->runFunctionInSyt(mainFn);
  ...
  uint64_t cacheQueries = cacheHits + cacheMisses;
  double cacheHitRate = 0.0;
  if (cacheQueries > 0)
    cacheHitRate = ((double) cacheHits) / ((double) cacheQueries);
  ...
  stats << '\n'
        ...
        << '\n'
        << "KLEE: done: error paths = "
        << handler->getNumErrorPaths()
        << '\n'
        << "KLEE: done: generated tests = " << handler->getNumTestCases()
        << '\n'
        << "KLEE: done: cache hits = " << cacheHits
        << '\n'
        << "KLEE: done: cache misses = " << cacheMisses
        << '\n'
        << "KLEE: done: cache hit rate = " 
          << std::fixed << std::setprecision(2) << cacheHitRate * 100 << "%"
        << '\n';

  if (handler->getNumLeakyPaths() > 0) {
    tpot_message("!!! --- Leaky paths: %d --- !!!", handler->getNumLeakyPaths());
  }
  ...
  std::string res;
  int exitCode = 0;
  if (handler->getNumErrorPaths() > 0) {
    res = "TPOT: Verification failed";
    exitCode = 1;
  } else {
    res = "TPOT: Verification succeeded";
  }

  if (handler->getNumLeakyPaths() > 0) {
    res += ", but there are memory leaks";
    exitCode = 1;
  }

  delete handler;

  // tpot_message("%s", res.c_str());
  llvm::errs() << res << '\n';
  return exitCode;
}
