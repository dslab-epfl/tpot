...
/*TPOT*/ struct usb_interface; // can't compile without this.
...
/*INTERNAL*/ /*@ predicate usb_device(struct usb_device *usb_device, struct usb_endpoint_descriptor *ep0;) =
//INTERNAL// 	usb_device_private(usb_device)
	...
	//
	//
//INTERNAL// 	&*& usb_endpoint_descriptor(ep0, USB_DIR_OUT, USB_ENDPOINT_XFER_CONTROL, ?pipe)
	...
	// XXX.
//INTERNAL// 	&*& usb_endpoint_descriptor_data(
//INTERNAL// 	&*& usb_device->product |-> ?product &*& string(product, _)
//INTERNAL// 	&*& usb_device->manufacturer |-> ?manufacturer &*& string(manufacturer, _)
//INTERNAL// 	&*& usb_device->serial |-> ?serial &*& string(serial, _)	
//INTERNAL// ;//SYNTAX//
//INTERNAL// @*///SYNTAX//


//INTERNAL// //@ predicate usb_device_private(struct usb_device *usb_device;);
...
//INTERNAL// //@ predicate current_thread_in_cb(int thread, bool deferred_data_xfer);
...
/*INTERNAL*/ typedef void usb_complete_t_no_pointer (struct urb *urb);
/*INTERNAL*/ 	/*@ requires
//INTERNAL// 		urb_struct(true,
//INTERNAL// 			urb, ?usb_dev, ?buffer, ?buffer_dma, ?buffer_alloc_size, ?user_alloc_dma, ?complete, ?context, ?setup
//INTERNAL// 		)//SYNTAX//
//INTERNAL// 		&*& buffer != 0
//INTERNAL// 		&*& permission_to_submit_urb(?urbs_submitted, true)
//INTERNAL// 		&*& complete_t_pred_fam(this)(?fracsize,
//INTERNAL// 			urb, usb_dev, buffer, buffer_dma,buffer_alloc_size, user_alloc_dma, complete, context, setup
//INTERNAL// 		)//SYNTAX//
//INTERNAL// 		&*& current_thread_in_cb(currentThread, false);
//INTERNAL// 	@*///SYNTAX//
//INTERNAL// 	/*@ ensures 
//INTERNAL// 		permission_to_submit_urb(_, true)
//INTERNAL// 		&*& current_thread_in_cb(currentThread, ?deferred_data_xfer)
//INTERNAL// 		&*& deferred_data_xfer ?
//INTERNAL// 			complete_t_pred_fam(this)(fracsize,
//INTERNAL// 				urb, usb_dev, buffer, buffer_dma,buffer_alloc_size, user_alloc_dma, complete, context, setup
//INTERNAL// 			)//SYNTAX//
//INTERNAL// 		:
//INTERNAL// 			complete_t_pred_fam_out(this)(fracsize,
//INTERNAL// 				urb, usb_dev, buffer, buffer_dma,buffer_alloc_size, user_alloc_dma, complete, context, setup
//INTERNAL// 			)//SYNTAX//
//INTERNAL// 		;
//INTERNAL// 	@*///SYNTAX//
...
typedef usb_complete_t_no_pointer* usb_complete_t; //INTERNAL// 
...
// edit some fields.
//INTERNAL// predicate urb_private(struct urb *urb, bool initialized, struct usb_device *dev, dma_addr_t data_dma, int actual_allocated_data_size, int flags, void *setup_packet;);
@*/
 ...
 */
/*INTERNAL*/ /*@ predicate urb_struct(
//INTERNAL// 	bool initialized,

//INTERNAL// 	struct urb *urb, struct usb_device *usb_dev, void *buffer, dma_addr_t buffer_dma, int buffer_alloc_size, bool user_alloc_dma, usb_complete_t complete, void *context, void *setup;
//INTERNAL// 	) =
...
//INTERNAL// 	[1/2]urb->status |-> ?status
...
//INTERNAL// 	&*& urb->dev |-> usb_dev
	
//INTERNAL// 	&*& urb->transfer_flags |-> ?data_transfer_flags
	
//INTERNAL// 	&*& urb->transfer_dma |-> buffer_dma
	
	// you can't overwrite it with non-working garbage, since
	// before submitting the URB, you have to have the complete_t_pred_fam.
//INTERNAL// 	&*& urb->context |-> context
...
//INTERNAL// 	&*& urb_private(urb, initialized, usb_dev, buffer_dma, buffer_alloc_size, data_transfer_flags, setup)
	
//INTERNAL// 	&*& (buffer != 0 ?
//INTERNAL// 		uchars(buffer, buffer_alloc_size, ?cs)
//INTERNAL// 	:true)
	
//INTERNAL// 	&*& (initialized ?
//INTERNAL// 		complete_t_ghost_param(complete, complete)
//INTERNAL// 	:true)
	
//INTERNAL// 	&*& (user_alloc_dma ?
//INTERNAL// 		buffer != 0

//INTERNAL// 		&*& true == ((data_transfer_flags | URB_NO_TRANSFER_DMA_MAP) == data_transfer_flags)
...
//INTERNAL// 		&*& usb_alloc_coherent_block(buffer, usb_dev, buffer_alloc_size, buffer_dma)
//INTERNAL// 	://SYNTAX//
//INTERNAL// 		true)
	
//INTERNAL// 	&*& setup != 0 ? // see comments in fill_control_urb
		//chars(setup, ?setup_packet_cs) // YYY
//INTERNAL// 		uchars(setup, 8, ?setup_packet_cs)
//INTERNAL// 	://SYNTAX//
//INTERNAL// 		true
		
//SYNTAX//;

//INTERNAL// predicate urb_struct_maybe(bool initialized, 
//INTERNAL// struct urb *urb, struct usb_device *usb_dev, void *buffer, dma_addr_t buffer_dma, int buffer_alloc_size, bool user_alloc_dma, usb_complete_t complete, void *context, void *setup;) =
//INTERNAL// 	urb == 0 ?
//INTERNAL// 		true // which means you can choose garbage for the other parameters.
//INTERNAL// 	:
//INTERNAL// 		urb_struct(initialized, urb, usb_dev, buffer, buffer_dma,buffer_alloc_size, user_alloc_dma, complete, context, setup)
//INTERNAL// 	;//SYNTAX//
...
//INTERNAL// predicate urb_submitted(
//INTERNAL// 	real fracsize,
//INTERNAL// 	struct urb *urb, struct usb_device *usb_dev, void *buffer, dma_addr_t buffer_dma, int buffer_alloc_size, bool user_alloc_dma, usb_complete_t complete, void *context, void *setup
//INTERNAL// );//SYNTAX//
...
//INTERNAL// 		true
//INTERNAL// 	:
//INTERNAL// 		urb_struct(false, result, ?dev, 0, ?data_dma, ?data_size, false,
//INTERNAL// 			0, // set to 0 in usb_init_urb (assuming memsetting 0 results in zeroed function pointers)
//INTERNAL// 			0, // analogues (assuming memsetting 0 results in int with value 0)
//INTERNAL// 			0
//INTERNAL// 		)//SYNTAX//
	;
@*/
...
/*INTERNAL*/ 	//@ requires urb_struct_maybe(?initialized, urb, ?dev, ?data, ?data_dma, ?data_size, ?has_dma, ?complete_fn, ?context, ?setup_packet);
//INTERNAL// 	/*@ ensures
//INTERNAL// 		urb == 0 ?
//INTERNAL// 			true // freeing zero.
//INTERNAL// 		:
//INTERNAL// 			(data != 0 ?
//INTERNAL// 				uchars(data, data_size, ?cs)
		
//INTERNAL// 				&*& has_dma?
//INTERNAL// 					usb_alloc_coherent_block(data, dev, data_size, data_dma)
//INTERNAL// 				:
//INTERNAL// 					true
//INTERNAL// 			:true)
//INTERNAL// 			
//INTERNAL// 			&*& (setup_packet != 0 ?
//INTERNAL// 				uchars(setup_packet, 8, ?setup_packet_cs)
//INTERNAL// 			:true)
//INTERNAL// 	;//SYNTAX//
//INTERNAL// 	@*///SYNTAX//

//INTERNAL// //@ predicate usb_alloc_coherent_block(void *ptr; struct usb_device *dev, int size, int dma);
...
//INTERNAL// 		&*& size >= 0 &*& integer(dma, ?ptr);
//INTERNAL// 	@*///SYNTAX//
/*INTERNAL*/ 	/*@ ensures 
		not_in_interrupt_context(currentThread)
//INTERNAL// 		&*& result == 0 ?
//INTERNAL// 			integer(dma, ptr)
//INTERNAL// 		:
//INTERNAL// 			usb_alloc_coherent_block(result, dev, size, ?ptr2) &*& integer(dma, ptr2)
//INTERNAL// 			&*& uchars(result, size, ?cs)
//INTERNAL// 		;//SYNTAX//
//INTERNAL// 	@*///SYNTAX//
...
//INTERNAL// 	/*@ requires
//INTERNAL// 		not_in_interrupt_context(currentThread) // might be unnecessarry
//INTERNAL// 		&*& addr == 0 ?
//INTERNAL// 			true
//INTERNAL// 		:
//INTERNAL// 			usb_alloc_coherent_block(addr, dev, size, dma)
//INTERNAL// 			&*& uchars(addr, size, ?cs)
//INTERNAL// 		;//SYNTAX//
//INTERNAL// 	@*///SYNTAX//
...
//INTERNAL// predicate usb_submit_urb_ghost_arg(bool deferred_data_xfer, real fracsize) = true;
@*/
...
//INTERNAL// 	&*& buffer != 0
//INTERNAL// 	&*& permission_to_submit_urb(?urbs_submitted, ?inside_completion)
//INTERNAL// 	&*& (mem_flags != GFP_ATOMIC ?
		not_in_interrupt_context(currentThread)
//INTERNAL// 	:
//INTERNAL// 		true
//INTERNAL// 	)//SYNTAX//
//INTERNAL// 	&*& usb_submit_urb_ghost_arg(?deferred_data_xfer, ?fracsize)
//INTERNAL// 	&*& deferred_data_xfer ?
		current_thread_in_cb(currentThread, false)
//INTERNAL// 	:
//INTERNAL// 		complete_t_pred_fam(complete)(fracsize,
//INTERNAL// 			urb, usb_dev, buffer, buffer_dma,buffer_alloc_size, user_alloc_dma, complete, context, setup
//INTERNAL// 		)//SYNTAX//
//INTERNAL// 	;//SYNTAX//
//INTERNAL// @*///SYNTAX//
...
//INTERNAL// 	&*& result == 0 ? // success
//INTERNAL// 		permission_to_submit_urb(urbs_submitted+1, inside_completion)
		&*& deferred_data_xfer ?
			current_thread_in_cb(currentThread, true)
		:
//INTERNAL// 			urb_submitted(
//INTERNAL// 				fracsize,
//INTERNAL// 				urb, usb_dev, buffer, buffer_dma,buffer_alloc_size, user_alloc_dma, complete, context, setup
//INTERNAL// 			)//SYNTAX//
	: // failure
...
//INTERNAL// 		)//SYNTAX//
		&*& deferred_data_xfer ?
			current_thread_in_cb(currentThread, false)
		:
//INTERNAL// 			complete_t_pred_fam(complete)(
//INTERNAL// 				fracsize,
//INTERNAL// 				urb, usb_dev, buffer, buffer_dma,buffer_alloc_size, user_alloc_dma, complete, context, setup
//INTERNAL// 			)//SYNTAX//
	;
...
//INTERNAL// 		true
//INTERNAL// 	:
//INTERNAL// 		urb_submitted(fracsize,
//INTERNAL// 			urb, usb_dev, buffer, buffer_dma,buffer_alloc_size, user_alloc_dma, complete, context, setup
//INTERNAL// 		)//SYNTAX//
//INTERNAL// 		&*& times_urb_submitted(times-1, fracsize,
//INTERNAL// 			urb, usb_dev, buffer, buffer_dma,buffer_alloc_size, user_alloc_dma, complete, context, setup
//INTERNAL// 		)//SYNTAX//
;

//INTERNAL// predicate times_complete_t_pred_fam_out (int times, real fracsize,
//INTERNAL// 	struct urb *urb, struct usb_device *usb_dev, void *buffer, dma_addr_t buffer_dma, int buffer_alloc_size, bool user_alloc_dma, usb_complete_t complete, void *context, void *setup
//INTERNAL// 	) =//SYNTAX//
...
//INTERNAL// 		true
	:
//INTERNAL// 		complete_t_pred_fam_out(complete)(fracsize,
//INTERNAL// 			urb, usb_dev, buffer, buffer_dma,buffer_alloc_size, user_alloc_dma, complete, context, setup
//INTERNAL// 		)//SYNTAX//
//INTERNAL// 		&*& times_complete_t_pred_fam_out(times-1,
//INTERNAL// 			fracsize,
//INTERNAL// 			urb, usb_dev, buffer, buffer_dma,buffer_alloc_size, user_alloc_dma, complete, context, setup
//INTERNAL// 		)//SYNTAX//
;
...
//INTERNAL// 	times_urb_submitted(?times, ?fracsize,
//INTERNAL// 		urb, ?usb_dev, ?buffer, ?buffer_dma, ?buffer_alloc_size, ?user_alloc_dma, ?complete, ?context, ?setup
//INTERNAL// 	)//SYNTAX//
	&*& not_in_interrupt_context(currentThread);
//INTERNAL// @*///SYNTAX//
/*INTERNAL*/ /*@ ensures
	not_in_interrupt_context(currentThread)
//INTERNAL// 	&*& times_complete_t_pred_fam_out(times, fracsize,
//INTERNAL// 		urb, usb_dev, buffer, buffer_dma,buffer_alloc_size, user_alloc_dma, complete, context, setup
//INTERNAL// 	)
	;
//INTERNAL// @*///SYNTAX//
...
/*@
//INTERNAL// predicate_family complete_t_pred_fam(usb_complete_t complete)(
//INTERNAL// 	real fracsize,
//INTERNAL// 	struct urb *urb, struct usb_device *usb_dev, void *buffer, dma_addr_t buffer_dma, int buffer_alloc_size, bool user_alloc_dma, usb_complete_t complete, void *context, void *setup
	
);

//INTERNAL// predicate_family complete_t_pred_fam_out(usb_complete_t complete)(
//INTERNAL// 	real fracsize,
//INTERNAL// 	struct urb *urb, struct usb_device *usb_dev, void *buffer, dma_addr_t buffer_dma, int buffer_alloc_size, bool user_alloc_dma, usb_complete_t complete, void *context, void *setup
//INTERNAL// 	);//SYNTAX//
@*/
 ...
 * same.
 */
//INTERNAL// /*@ predicate complete_t_ghost_param(usb_complete_t_no_pointer *complete_no_ptr, usb_complete_t complete;) =
//INTERNAL// 	is_usb_complete_t_no_pointer(complete) == true
//INTERNAL// 	&*& complete_no_ptr == complete;
...
//INTERNAL// 	/*@ requires urb_struct(_, urb, _, 0, ?data_dma, ?data_size, ?data_transfer_flags, ?original_complete_fn, ?original_context, ?setup_packet)
//INTERNAL// 		&*& [?f]usb_device(dev, ?ep0)
//INTERNAL// 		&*& [?f2]usb_endpoint_descriptor(?epd, USB_DIR_IN, ?xfer_type, pipe)
//INTERNAL// 		&*& transfer_buffer != 0
//INTERNAL// 		&*& uchars(transfer_buffer, ?length_transfer_buffer, ?cs)
//INTERNAL// 		&*& length_transfer_buffer >= buffer_length
//INTERNAL// 		&*& complete_t_ghost_param(complete_fn, complete_fn)
//INTERNAL// 		;//SYNTAX//
//INTERNAL// 	@*///SYNTAX//
...
//INTERNAL// 	/*@ ensures urb_struct(true, urb, dev, transfer_buffer, data_dma, length(cs), data_transfer_flags, complete_fn, context, setup_packet)
//INTERNAL// 		&*& [f]usb_device(dev, ep0)
//INTERNAL// 		&*& [f2]usb_endpoint_descriptor(epd, USB_DIR_IN, xfer_type, pipe);
//INTERNAL// 	@*///SYNTAX//
...
//INTERNAL// /*@ predicate usb_host_interface(struct usb_host_interface *usb_host_interface) = 
//INTERNAL// 	usb_interface_descriptor(& usb_host_interface->desc, ?bNumEndpoints, ?bInterfaceNumber) &*&
//INTERNAL// 	bNumEndpoints > 0 ?
		// This is actually an array, we currently only support the
		// first element here (if there is a first element).
//INTERNAL// 		[?f]usb_host_interface->endpoint |-> ?endpoint
//INTERNAL// 		&*& usb_host_endpoint(endpoint)
//INTERNAL// 	:
//INTERNAL// 		true
	;
//INTERNAL// @*///SYNTAX//
...
//INTERNAL// //@ predicate usb_interface_private(struct usb_interface *usb_interface, struct usb_device *usb_device, void *data, bool has_data, real probe_disconnect_fraction_size);
//INTERNAL// /*@ predicate usb_interface(
//INTERNAL// 		vf_usb_operation_probe_t *probe_cb,
//INTERNAL// 		vf_usb_operation_disconnect_t *disconnect_cb,
//INTERNAL// 		struct usb_interface *usb_interface, struct usb_device *usb_device, void *data, bool has_data, real probe_disconnect_fraction_size
//INTERNAL// 	) =
	// Fraction of specified size to disallow stealing fractions and writing.
//INTERNAL// 	[1/2] usb_interface->cur_altsetting |-> ?cur_altsetting
//INTERNAL// 	&*& usb_host_interface(cur_altsetting)
	
//INTERNAL// 	&*& usb_device(usb_device, ?ep0)
//INTERNAL// 	&*& usb_interface_private(usb_interface, usb_device, data, has_data, probe_disconnect_fraction_size)
...
//INTERNAL// 		userdef_usb_interface_data(probe_cb, disconnect_cb)(usb_interface,usb_device, data, probe_disconnect_fraction_size)
//INTERNAL// 	:
//INTERNAL// 		true
//INTERNAL// 	;//SYNTAX//
//INTERNAL// @*///SYNTAX//
...
//INTERNAL// 	&*& data != 0 ?
//INTERNAL// 		userdef_usb_interface_data(probe_cb, disconnect_cb)(intf, dev, data, fracsize)
//INTERNAL// 	:
//INTERNAL// 		true
//INTERNAL// 	;//SYNTAX//
//INTERNAL// @*///SYNTAX//	
...
//INTERNAL// 	&*& usb_interface(probe_cb, disconnect_cb, intf, dev, data, data != 0, fracsize)
//INTERNAL// 	&*& ( originalHasData ?
//INTERNAL// 		userdef_usb_interface_data(probe_cb, disconnect_cb)(intf, dev, originalData, fracsize)
//INTERNAL// 	:
//INTERNAL// 		true
//INTERNAL// 	);//SYNTAX//
...
/*INTERNAL*/ /*@ requires usb_interface(?open_cb, ?close_cb, intf, ?dev, ?data, ?hasData, ?fracsize);
@*/
/*INTERNAL*/ /*@ ensures usb_interface(open_cb, close_cb, intf, dev, data, hasData, fracsize) &*& result == data;
@*/


struct usb_device *interface_to_usbdev(struct usb_interface *intf);
//INTERNAL// 	//@ requires [?frac_intf] usb_interface(?open_cb, ?close_cb, intf, ?usb_device, ?data, ?hasData, ?fracsize);
//INTERNAL// 	/*@ ensures [frac_intf] usb_interface(open_cb, close_cb, intf, result, data, hasData, fracsize)
//INTERNAL// 		&*& result != 0
//INTERNAL// 		&*& result == usb_device
//INTERNAL// 		;//SYNTAX//
//INTERNAL// 	@*///SYNTAX//
...
//INTERNAL// 	/*@ requires [?f]usb_device(udev, ?ep0) &*& [?f2]usb_endpoint_descriptor(?desc, ?dir, ?xfer_type, ?pipeOption)
//INTERNAL// 		&*& pipeOption == pipe

		// Linux's USB implementation does this check at runtime. We enforce it here.
//INTERNAL// 		&*& is_out != 0 ?
//INTERNAL// 			dir == USB_DIR_OUT
//INTERNAL// 		:
//INTERNAL// 			dir == USB_DIR_IN
//INTERNAL// 		;//SYNTAX//
//INTERNAL// 	@*///SYNTAX//
//INTERNAL// 	/*@ ensures
//INTERNAL// 		[f]usb_device(udev, ep0) &*& [f2]usb_endpoint_descriptor(desc, dir, xfer_type, pipe);
//INTERNAL// 	@*///SYNTAX//
...
/*INTERNAL*/   //@ requires chars(buf, ?bufsize, ?old_text) &*& size <= bufsize &*& usb_device_private(dev);
/*INTERNAL*/   //@ ensures chars(buf, bufsize, ?new_text) &*& usb_device_private(dev) &*& ! mem('\0', old_text) || mem('\0', new_text);
