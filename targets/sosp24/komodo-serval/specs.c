...
uint64_t _payload_start[KOM_INSECURE_NPAGES][KOM_PAGE_SIZE / sizeof(uint64_t)];
...
bool does_not_reference(uint64_t *unused, int64_t i, kom_secure_pageno_t pageno) {
  ...
  return i == pageno || g_pagedb[i].addrspace_page != pageno;
}
...
void spec_kom_smc_remove_addrspace() {
  ...
  assert(g_pagedb[pageno].addrspace_page == -1);
...
void spec_kom_smc_map_insecure() {
  ...
  int result = kom_smc_map_insecure(l3pt_page, l3pt_index, mapping, insecure_pageno);
  assert(result == KOM_ERR_SUCCESS);
  pte_t entry = pfn_pte(PFN_DOWN((uintptr_t)&_payload_start[insecure_pageno]),
...
void spec_kom_smc_map_secure_success() {
  ...
  uint64_t addrspace_page = g_pagedb[l3pt_page].addrspace_page;
  struct kom_addrspace *address = (struct kom_addrspace *)&secure_pages[addrspace_page];
  if (g_pagedb[addrspace_page].type != KOM_PAGE_ADDRSPACE || address->state != KOM_ADDRSPACE_INIT) {
    return;
  }

  uint64_t old_refcnt = address->refcount;

  assert(g_pagedb[addrspace_page].type == KOM_PAGE_ADDRSPACE);

  int result = kom_smc_map_secure(page, l3pt_page, l3pt_index, mapping, content);
  assert(result == KOM_ERR_SUCCESS);
  assert(address->refcount == old_refcnt + 1);
  assert(g_pagedb[page].type == KOM_PAGE_DATA);
  assert(g_pagedb[page].addrspace_page == addrspace_page);
  pte_t entry =
...
void spec_kom_smc_init_l2ptable() {
  ...
  if (l1pt_index >= KOM_PAGE_SIZE / sizeof(uint64_t)) {
...
void spec_kom_smc_init_addrspace() {
  ...
  struct kom_addrspace *address = (struct kom_addrspace *)&secure_pages[addrspace_page];
  assert(g_pagedb[addrspace_page].type == KOM_PAGE_ADDRSPACE);
  assert(g_pagedb[addrspace_page].addrspace_page == addrspace_page);
  assert(g_pagedb[l1pt_page].type == KOM_PAGE_L1PTABLE);
  assert(g_pagedb[l1pt_page].addrspace_page == addrspace_page);
  assert(address->l1pt_page == l1pt_page);
  assert(address->refcount == 2);
  assert(address->state == KOM_ADDRSPACE_INIT);
}