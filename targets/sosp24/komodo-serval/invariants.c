...
bool addrspace_page_valid(uint64_t *unused, int64_t i) {
  ...
  if (ptr->addrspace_page < 0 || ptr->addrspace_page >= KOM_SECURE_NPAGES) {
    return false;
  }
  ...
  if (ptr->type != KOM_PAGE_ADDRSPACE) {
    ...
    if (g_pagedb[ptr->addrspace_page].type != KOM_PAGE_ADDRSPACE) {

      return false;
    }
    // no self-references
    if (ptr->addrspace_page == i)
      return false;
  }
...
bool insecure_l1page_valid(uint64_t *unused, int64_t i) {
  if (i < 0 || i >= KOM_INSECURE_NPAGES)
    return true;
  struct kom_addrspace *ptr = (struct kom_addrspace *)&_payload_start[i];
  return ptr->l1pt_page < KOM_INSECURE_NPAGES;
}

bool inv__insecure_page_valid() {
  return forall_elem_(_payload_start, insecure_l1page_valid);
}
...
bool pgtable_ok(int64_t pgno, int64_t index) {
  ...
  return g_pagedb[g_pagedb[pgno].addrspace_page].type == KOM_PAGE_ADDRSPACE &&
          ((struct kom_addrspace *)&secure_pages[g_pagedb[pgno].addrspace_page])->state != KOM_ADDRSPACE_STOPPED;
}
...
bool page_table_valid(uint64_t *unused, int64_t i) {
  ...
  if (ptr->type == KOM_PAGE_L1PTABLE && 
    ...
    if (pgd_present(pgd)) {
      ...
      if (l2->addrspace_page != ptr->addrspace_page) {
        return false;
      }
    }
  }
  ...
  if (ptr->type == KOM_PAGE_L2PTABLE && 
    ...
    if (pmd_present(pmd)) {
      ...
      if (l3->addrspace_page != ptr->addrspace_page) {
        return false;
      }
    }
  }
  ...
  if (ptr->type == KOM_PAGE_L3PTABLE && 
    ...
    if (pte_present(pte)) {
      ...
      if (pte_pfn(pte) >= KOM_INSECURE_NPAGES) {
        ...
        if (p->addrspace_page != ptr->addrspace_page) {
          return false;
        }
      } else {
